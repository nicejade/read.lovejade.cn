...
export default {
  ...
  methods: {
    ...
    wakeUpQuickappByDeeplink() {
      const protocol = this.isWechatBrowser() ? 'https://hapjs.org/' : 'https://hapjs.org/' // 'hap://'
      const aNodeDom = document.createElement('a')
      aNodeDom.setAttribute('href', `${protocol}app/com.graceful.sentences/pages/Main`)
    ...
    wakeUpQuickappByUrl() {
      ...
      fetch(`http://thefatherofsalmon.com/?i=com.graceful.sentences&random=${random}`)
        .then(res => res.json())
